...
class node:
    def __init__(self, x, y, value, height, visited = False, parent = None):
        self.x = x
        self.y = y
        self.value = value,
        self.height = height
...
def bfs(s, e, grid):
    ...
    while len(queue) > 0:
        ...
        values = [neighbour for neighbour in validNeighbours if not(neighbour.visited)]
...
for part2in in part2ins:
    ...
    getParent(res)
    if res != None:
        part2outs.append(len(ans))

print(sorted(part2outs)[0]-1)
