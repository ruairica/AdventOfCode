...
public class Tasks2021
{
    ...
    public void day3_2()
    {
        ...
        {
            var maxesDict = filteredMaxes.GroupBy(x => x[i])
            ...
            {
                filteredMaxes = maxesDict.MaxBy(x => x.Value.Count).Value;
            }
        ...
        for (int i = 0; i < numbers[0].Length; i++)
        {
            var minsDict = filteredMins.GroupBy(x => x[i])
                .ToDictionary(x => x.Key, x => x.ToList());
            ...
            {
                filteredMins = minsDict.MinBy(x => x.Value.Count).Value;
            }
        ...
        Console.WriteLine(
            Helpers.ConvertToDecimal(
                finalMax.Select(x => int.Parse(x.ToString())).ToList()) *
            Helpers.ConvertToDecimal(
                finalMin.Select(x => int.Parse(x.ToString())).ToList()));
    }
    ...
    public void day4_1_2()
    {
        ...
        var bingoCardSplits = splits[1..].Select(x => x.Replace("  ", " ").Trim());
    ...
    public void day4_1_grid()
    {
        ...
        var cards = bingoCardSplits
            .Select(
                x => x.Split('\n')
                    .Select(x => x.Trim().Split(' ').Select(int.Parse).ToList())
                    .ToList())
            .ToList()
            .Select(x => new GridWithOtherVal<bool>(x, false))
            .ToList();
        ...
        foreach (var call in bingoCalls.Split(',').Select(int.Parse))
        {
            foreach (var card in cards)
            {
                ...
                if (bingo)
                {
                    (call * card.WhereWithCoord((gi, coord) => !gi.otherVal)
    ...
    {
        var textLines = File.ReadAllText("./inputs/day5.txt").Trim().Split("\n").ToList();

        var lines = textLines.Select(
                l =>
                {
        ...
        foreach (var line in lines)
        {
            ...
            if (line.startX != line.endX && line.startY != line.endY)
            {
                ...
                var diagPoints = Enumerable.Range(x1, xDiff)
                    .Select(_ => (xPos ? x1++ : x1--, yPos ? y1++ : y1--));
    ...
    public void day6_2_ListInsteadOfDict()
    {
        ...
        var fish = File.ReadAllLines("./inputs/day6.txt")
            ...
            .GroupBy(f => f)
            .Aggregate(
                new List<long>(new long[newFishResetDay]),
                (list, kv) =>
                {
    ...
    public void day7_2()
    {
        ...
        {
            var currentFuel = positions.Sum(
                p => Enumerable.Range(0, Math.Abs(p - pos) + 1).Sum(x => 1 * x));
        ...
        File.ReadAllText("./inputs/day8.txt")
            ...
            .Select(
                line =>
                {
                    ...
                    var signals = s.Trim()
                        .Split(' ')
                        .Select(x => string.Join("", x.OrderBy(x => x)));
                    ...
                    return long.Parse(
                        string.Join("", (lineOut.Select(x => numberMatches.IndexOf(x)))));
                })
            .Sum()
            .Dump();
    }
 ...
 */
        ...
        for (var x = 0; x < grid.Count; x++)
        {
            for (var y = 0; y < grid[0].Count; y++)
            {
                ...
                if (new List<(int, int)> { new(0, 1), new(0, -1), new(1, 0), new(-1, 0) }
                    .Where(
                        e => e.Item1 + x >= 0 && e.Item1 + x < grid.Count &&
                             e.Item2 + y >= 0 && e.Item2 + y < grid[0].Count)
    ...
    public void day9_1_Linqier()
    {
        ...
        Enumerable.Range(0, grid.Count)
            .SelectMany(
                x => Enumerable.Range(0, grid[x].Count)
                    .Select(
                        y =>
                            new List<(int, int)>
                                {
        ...
        foreach (var (bx, by) in basins)
        {
            ...
            while (tail.Count > 0)
            {
                ...
                new List<(int, int)> { new(0, 1), new(0, -1), new(1, 0), new(-1, 0) }
                    .Where(
                        e => e.Item1 + cx >= 0 && e.Item1 + cx < grid.Count &&
                             e.Item2 + cy >= 0 && e.Item2 + cy < grid[0].Count)
                    .Where(e => grid[cx + e.Item1][cy + e.Item2] != 9)
                    .ToList()
                    .ForEach(e => { tail.Enqueue(new(cx + e.Item1, cy + e.Item2)); });
            }

            basinTotals.Add(visited.Count);
        }

        basinTotals.OrderByDescending(x => x)
            .Take(3)
    ...
    public void day9_2_grid()
    {
        ...
        var basinCoords = grid
            .WhereWithCoord(
                (v, coord) => grid.GetValidAdjacentNoDiag(coord).All(a => grid[a] > v))
            ...
            .Select(
                line =>
                {
                    ...
                    return stack.Aggregate(
                        (long)0,
                        (score, letter) => score * 5 + scores[matches[letter]]);
                })
            .OrderBy(x => x)
            .Median()
            .Dump();
    ...
    public void day11_1()
    {
        var grid = File.ReadAllText("./inputs/day11.txt")
            .Replace("\r\n", "\n")
            .Trim()
            .Split("\n")
    ...
    {
        var g = File.ReadAllText("./inputs/day11.txt")
            .Replace("\r\n", "\n")
            .Trim()
            .Split("\n")
            .Select(x => x.ToCharArray().Select(x => int.Parse(x.ToString())).ToList())
        ...
        for (int step = 1; step <= 100; step++)
        {
            ...
            var visited = new HashSet<Coord>();
            // increase all by one, if it's >9 reset to 0
            grid = grid.SetValues(x => x + 1).SetValues(x => x > 9 ? 0 : x);
    ...
    public void day12_1()
    {
        ...
        File.ReadAllText("./inputs/day12.txt")
            ...
            .ForEach(
                line =>
                {
                    ...
                    links[cave1] = links.GetValueOrDefault(cave1, new List<string>())
                        .Concat(new List<string> { cave2 })
                        .ToList();

                    links[cave2] = links.GetValueOrDefault(cave2, new List<string>())
                        .Concat(new List<string> { cave1 })
                        .ToList();
        ...
        while (stack.Any())
        {
            ...
            if (smallCaves.Contains(currentCave.cave) &&
            ...
            linkCaves.ForEach(
                x => stack.Push(
                    new(
                        x,
                        currentCave.pathSoFar
                            .Concat(new List<string> { currentCave.cave })
                            .ToList())));
        }
    ...
    public void day12_2()
    {
        ...
        var stack =
            new Stack<(string cave, List<string> pathSoFar, bool hasUsedDuplicate)>();
        ...
        while (stack.Any())
        {
            ...
            if ((smallCaves.Contains(currentCave.cave) &&
                 currentCave.pathSoFar.Contains(currentCave.cave) &&
                 currentCave.hasUsedDuplicate) || (currentCave.cave == startCave &&
                                                   currentCave.pathSoFar.Contains(
                                                       currentCave.cave)))
            {
                continue;
            }

            var shouldHaveDup = currentCave.hasUsedDuplicate ||
                                (smallCaves.Contains(currentCave.cave) &&
                                 currentCave.pathSoFar.Contains(currentCave.cave) &&
            ...
            linkCaves.ForEach(
                x => stack.Push(
                    new(
                        x,
                        new List<string>(currentCave.pathSoFar) { currentCave.cave },
                        shouldHaveDup)));
        }
    ...
    public void day13_1()
    {
        var (pointLines, folds) = File.ReadAllText("./inputs/day13.txt")
            .Trim()
            .Replace("\r\n", "\n")
            .Split("\n\n");

        var points = pointLines.Split("\n")
            .Select(
                pl =>
                {
        ...
        if (dir == "x")
        {
            ...
            foreach (var (ox, oy) in points.Where(kvp => kvp.Key.Item1 > lineNum)
                         .ToList()
                         .Select(x => x.Key))
            {
    ...
    public void day13_2()
    {
                     ...
                     .Select(l => l.Split(' ').Last().Split('=')))
        {
            ...
            foreach (var (ox, oy) in points
                         .Where(
                             kvp => horizontal
                                 ? kvp.Key.Item1 > lineNum
                                 : kvp.Key.Item2 > lineNum)
                         .ToList()
                         .Select(x => x.Key))
            {
    ...
    public void day14_1()
    {
        var (original, r) = File.ReadAllText("./inputs/day14.txt")
            .Trim()
            .Replace("\r\n", "\n")
            .Split("\n\n");
    ...
    public void day14_2()
    {
        ...
        var finalFreqs = finals.Where(x => x.Value > 0)
            .Select(x => x.Value / 2)
            .OrderBy(x => x)
            .ToList();
    ...
    public void day15_1_2021_Dijkstra()
    {
        ...
        var q = new PriorityQueue<(Coord, int cost), int>();
        q.Enqueue((start, 0), 0);
        var seen = new HashSet<Coord>();

        while (q.Count > 0)
        {
            var (current, cost) = q.Dequeue();
            ...
            grid.GetValidAdjacentNoDiag(current)
                .ForEach(
                    adj =>
                    {
                        var newCost = cost + grid[adj];
    ...
    [Test]
    public void day15_2_2021_Dijkstra()
    {
        ...
        var start = new Coord(0, 0);
        var end = new Coord(g.Height - 1, g.Width - 1);
        ...
        while (q.Count > 0)
        {
            ...
            g.GetValidAdjacentNoDiag(current)
                .ForEach(
                    adj =>
                    {
                        var newCost = cost + g[adj];
                        q.Enqueue((adj, newCost), newCost);
                    });
        }
    }
    ...
    public void day16_1()
    {
        ...
        var inputString = File.ReadAllText("./inputs/day16.txt").Trim();
    ...
    public class BingoCard
    {
        ...
        {
            return this.Numbers.SelectMany(x => x)
                .Where(x => !x.marked)
                .Select(x => x.number)
                .Sum();
        }
        ...
        private List<List<(int number, bool marked)>> GetAllColumns()
        {
            return Enumerable.Range(0, this.Numbers[0].Count)
                .Select(
                    column => Enumerable.Range(0, this.Numbers.Count)
                        .Select(row => this.Numbers[row][column])
                        .ToList())
                .ToList();
        }
    }
